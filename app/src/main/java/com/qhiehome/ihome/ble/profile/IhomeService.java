...
import com.qhiehome.ihome.ble.request.NotificationRunnable;
...
import com.qhiehome.ihome.service.IhomeServiceBase;
...
public class  IhomeService extends IhomeServiceBase implements Observer{

    private static final String TAG = "IhomeService";
    ...
    private final int HEART_BEAT = 10009;
    ...
    private TimeoutRunner mFirstTimeConnTimeout;
    ...
    public void onCreate() {
        super.onCreate();
        mContext = this;
        LogUtil.d(TAG, "IhomeService onCreate()");
        if (!initBluetooth()) {
            ToastUtil.showToast(this, "BluetoothAdapter init failed");
            LogUtil.e(TAG, "BluetoothAdapter init failed");
        }
                ...
                switch (msg.what) {
                    case HEART_BEAT:
                        CommunicationManager.getInstance()
                                .sendBLEEvent(mContext, new Intent(BLECommandIntent.ACTION_HEART_BEAT));
                        if (CommunicationManager.getInstance().isBleConnected()) {
                            sendEmptyMessageDelayed(HEART_BEAT, 2000);
                        }
                        break;
                    default:
                        break;
        ...
        mFirstTimeConnTimeout = new TimeoutRunner(TimeoutRunner.DEVICE_FIRST_TIME_CONNECT);
    ...
    public void onDestroy() {
        super.onDestroy();
        LogUtil.d(TAG, "IhomeService onDestroy");
        ...
        if (intent != null) {
            mGattHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    boolean isBleConnectingOrConnected = CommunicationManager.getInstance().isBleConnectingOrConnected();
                    if (action.equals(CommunicationManager.ACTION_CONNECT_TO_DEVICE)) {
                        if (bundle == null || (!bundle.containsKey(CommunicationManager.EXTRA_ADDRESS))) {
                            LogUtil.e(TAG, "no address info in the bundle");
                            return;
                        }
                        String address = bundle.getString(CommunicationManager.EXTRA_ADDRESS);
                        String name = bundle.getString(CommunicationManager.EXTRA_NAME);
                        if (!isDeviceValid(address)) {
                            LogUtil.e(TAG, "address " + address + " is invalid");
                            return;
                        }
                        mGattHandler.removeCallbacks(mFirstTimeConnTimeout);

                        if (isBleConnectingOrConnected) {
                            LogUtil.d(TAG, "Action is connect. But we already in connecting/connected state, disconnect with it frist!");
                            disconnect(true);
                        }
                        // check if the device is first time to connect
                        isFirstConnected = (null == DatabaseHelper.getInstance().queryLockBean(address));
                        if (!isFirstConnected) {
                            // query device in untrustted locks
                            int timeToNow = DatabaseHelper
                                    .getInstance()
                                    .findInUntrusttedLocks(address);
                            if (timeToNow > 0) {
                                LogUtil.d(TAG, "in UntrusttedLocks list...");
                                ToastUtil.showToast(IhomeService.this, "Password incorrect many times");
                                showDisconnectedNotification();
                                CommunicationManager
                                        .getInstance()
                                        .sendBTStateChanged(mContext, CommunicationManager.BLE_DISCONNECTED);
                                removeAllTimeoutRunners();
                                return;
                            }
                        }
                        mDeviceAddress = address;
                        mDeviceName = name;
                        connectToBLE(true, isFirstConnected);
                    } else if (CommunicationManager.ACTION_UI_LAUNCH.equals(action)) {
                        LogUtil.d(TAG, "Action is CommunicationManager.ACTION_UI_LAUNCH--- mAddress: " + mDeviceAddress + ", isConnecting: " + isBleConnectingOrConnected);
                    } else if (CommunicationManager.ACTION_SERVICE_CONFIG_SUCCESS.equals(action)) {
                        CommunicationManager.getInstance().DEVICE_REAL_NAME = mBluetoothDevice.getName();
                        CommunicationManager.getInstance().sendBTStateChanged(mContext, CommunicationManager.BLE_CONNECTED);
                        showConnectedNotification();
                        notifyClientServices(true);
                        mGattHandler.sendEmptyMessage(HEART_BEAT);
                        LogUtil.d(TAG, "CommunicationManager.ACTION_SERVICE_CONFIG_SUCCESS receive!!!");
                    } else if (CommunicationManager.ACTION_RX_STATUS_SUCCESS.equals(action)) {
                        removeDeviceFristConnTimeoutRunner();
                    } else if (CommunicationManager.ACTION_SERVICE_CONFIG_FAIL.equals(action)) {
                        if (isBleConnectingOrConnected) {
                            reconnect();
                        }
                    } else if (CommunicationManager.ACTION_DISCONNECT_TO_DEVICE.equals(action)) {
    ...
    private void connectToBLE(boolean scanFrist) {
        ...
        mGattHandler.removeMessages(HEART_BEAT);
    ...
    private void connectToBLE(boolean scanFirst, boolean isFirstConnected) {
        if (isFirstConnected) {
            LogUtil.e(TAG, "This is a new device, first time connect, start time out runner...");
            removeRequestTimeoutRunner();
            mFirstTimeConnTimeout = new TimeoutRunner(
                    TimeoutRunner.DEVICE_FIRST_TIME_CONNECT);
            mGattHandler
                    .postDelayed(mFirstTimeConnTimeout, TimeoutRunner.DEVICE_FIRST_TIME_CONNECT_DELAY);
            sendBroadcast(new Intent(CommunicationManager.ACTION_DEVICE_FIRST_TIME));
        }
        connectToBLE(scanFirst);
    }

    private boolean isDeviceValid(String address) {
        if (!isAddressValid(address)) {
            LogUtil.e(TAG, "Device address is invalid");
            return false;
        }
        LogUtil.d(TAG, "Device MAC: " + address + " is valid");

        mBluetoothDevice = mBluetoothAdapter.getRemoteDevice(address);
        if (mBluetoothDevice == null) {
            LogUtil.e(TAG, "Failed to find the bluetooth device");
            ToastUtil.showToast(this, "No device");
            return false;
        }
        return true;
    }
    ...
    private boolean isAddressValid(String address) {
        if ((!TextUtils.isEmpty(address)) && (BluetoothAdapter
    ...
    private class TimeoutRunner implements Runnable {
        ...
        public static final int DEVICE_FIRST_TIME_CONNECT_DELAY = 11000;

        public static final int STATE_CONNECTING = 3;
        public static final int STATE_DISCOVERING = 1;
        public static final int STATE_REQUESTING = 2;
        public static final int DEVICE_FIRST_TIME_CONNECT = 4;
        ...
        public void run() {
            switch (mState) {
                ...
                case STATE_DISCOVERING:
                    ...
                    handleDiscoveryFail();
                    break;
                case DEVICE_FIRST_TIME_CONNECT:
                    LogUtil.e(TAG, "new device, frist time connect, time out ...");
                    sendBroadcast(new Intent(CommunicationManager.ACTION_DEVICE_FIRST_TIMEOUT));
                    disconnectByHost();
                    break;
            }
        }
    ...
    private void removeDeviceFristConnTimeoutRunner() {
        if (mFirstTimeConnTimeout != null) {
            LogUtil.e(TAG, "removeDeviceFristConnTimeoutRunner... ");
            mGattHandler.removeCallbacks(mFirstTimeConnTimeout);
            mFirstTimeConnTimeout = null;
        }
    }

    private void disconnectByHost() {
        LogUtil.i(TAG, "entr disconnectByHost...");
    ...
    private class GattConntionStateChangeRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (profileState == BluetoothGatt.STATE_CONNECTED) {
                ...
                mRequestManager.clearRequestQueue();
                LockBean lock = DatabaseHelper.getInstance()
                        .queryLockBean(mDeviceAddress);
                ...
                if (mBluetoothGatt != null) {
                    if (!CommunicationManager.getInstance().HOST_DISCONNECT && (lock == null || lock
                            .getRole() == LockBean.LOCK_ROLE.OWNER.ordinal())) {
                        try {
    ...
    private void afterScanAction() {
        ...
        mConnectTimeout = new TimeoutRunner(TimeoutRunner.STATE_CONNECTING);
        mGattHandler
    ...
    private void failCurrentRequest() {
        if ((mCurrentRequest != null) && (mCurrentRequest.getmRequestCallback() != null)) {
            // mGattHandler.removeCallbacks(mRequestTimeout);
            mCurrentRequest
                    .getmRequestCallback()
                    .onRequestComplete(mCurrentRequest.getmCharacteristic(), false, mCurrentRequest
                            .getmType());
        }
    ...
    private void removeAllTimeoutRunners() {
    ...
    private void getNewRequest() {
        ...
        if (mBluetoothGatt == null) {
            ...
            if (mCurrentRequest != null) {
                mCurrentCharacteristic = mCurrentRequest.getmCharacteristic();
    ...
    private void handleRequestResult(BluetoothGattCharacteristic characteristic, boolean isSuccess) {
        ...
        if ((mCurrentRequest != null) && (mCurrentRequest.getmRequestCallback() != null)) {
            if (characteristic == null) {
                characteristic = mCurrentRequest.getmCharacteristic();
                isSuccess = false;
            }
            mCurrentRequest
                    .getmRequestCallback()
    ...
    private class GattReadOrWriteRunnable implements Runnable {
        ...
        public void run() {
            if (mConnectiongStatus != BluetoothGatt.GATT_SUCCESS && mCurrentRequest != null && mCurrentRequest
                    .getmType() == Request.REQUEST_TYPE.WRITE) {
                // restart();
                failCurrentRequest();
    ...
    private class GattDescriptorReadOrWriteRunnable implements Runnable {
        ...
        public void run() {
            if (mConnectiongStatus != BluetoothGatt.GATT_SUCCESS) {
                if (mCurrentRequest != null && mCurrentRequest.getmType() == Request.REQUEST_TYPE.WRITE) {
                    return;
                }
                failCurrentRequest();
